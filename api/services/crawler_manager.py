...
class CrawlerManager:
    ...
    def _init_file_logger(self) -> logging.Logger:
        file_logger = logging.getLogger("MediaCrawler.WebUI")
        file_logger.setLevel(logging.INFO)
        file_logger.propagate = False
        if not any(getattr(handler, "_mediacrawler_webui_handler", False) for handler in file_logger.handlers):
            try:
                output_dir = self._project_root / "output"
                output_dir.mkdir(parents=True, exist_ok=True)
                file_handler = RotatingFileHandler(
                    output_dir / "webui.log",
                ...
                file_handler._mediacrawler_webui_handler = True
    ...
    async def add_log(self, message: str, level: str = "info") -> LogEntry:
        """Add a system log entry and broadcast it to connected WebUI clients."""
    ...
    def _create_log_entry(self, message: str, level: str = "info") -> LogEntry:
        """Create log entry"""
        self._log_id += 1
        self._file_logger.info("[%s] %s", level.upper(), message)
        entry = LogEntry(
            id=self._log_id,
