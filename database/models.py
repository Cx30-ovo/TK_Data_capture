...
class DouyinMonitoredAccount(Base):
    __tablename__ = 'douyin_monitored_accounts'
    __table_args__ = (
        UniqueConstraint('platform', 'sec_user_id', name='uq_douyin_monitor_account'),
    )
    ...
    last_discovered_at = Column(BigInteger, index=True, comment='最近一次发现时间戳')
...
class DouyinPost(Base):
    ...
    __table_args__ = (
        UniqueConstraint('platform', 'aweme_id', name='uq_douyin_post'),
    )

    id = Column(Integer, primary_key=True, comment='主键ID')
    platform = Column(String(32), nullable=False, default='dy', index=True, comment='平台标识')
    aweme_id = Column(String(128), nullable=False, index=True, comment='作品ID')
    sec_user_id = Column(String(255), nullable=False, index=True, comment='所属账号安全标识')
    title = Column(Text, comment='作品标题')
    desc = Column(Text, comment='作品正文')
...
class DouyinPostSnapshot(Base):
    __tablename__ = 'douyin_post_snapshots'
    __table_args__ = (
        UniqueConstraint('platform', 'aweme_id', 'stage', name='uq_douyin_post_snapshot_stage'),
    )

    id = Column(Integer, primary_key=True, comment='主键ID')
    platform = Column(String(32), nullable=False, default='dy', index=True, comment='平台标识')
    aweme_id = Column(String(128), nullable=False, index=True, comment='作品ID')
    stage = Column(String(16), nullable=False, index=True, comment='快照阶段: 1h/6h/24h/72h/7d')
    due_at = Column(BigInteger, nullable=False, index=True, comment='计划采集时间戳')
    captured_at = Column(BigInteger, nullable=False, index=True, comment='实际观测时间戳')
    actual_age_seconds = Column(BigInteger, nullable=False, comment='实际发布后年龄，单位秒')
    ...
    created_at = Column(BigInteger, nullable=False, comment='写入时间戳')


class DouyinMonitorJob(Base):
    __tablename__ = 'douyin_monitor_jobs'
    ...
    due_at = Column(BigInteger, nullable=False, index=True, comment='到期时间戳')
    status = Column(String(16), nullable=False, default='pending', index=True, comment='状态: pending/running/done/failed/missed/skipped')
    attempts = Column(Integer, nullable=False, default=0, comment='已尝试次数')
    last_error = Column(Text, comment='最近错误')
    miss_reason = Column(Text, comment='missed或failed原因')
    created_at = Column(BigInteger, nullable=False, comment='创建时间戳')
    started_at = Column(BigInteger, comment='开始执行时间戳')
    finished_at = Column(BigInteger, comment='完成时间戳')


class DouyinTopicProfile(Base):
    __tablename__ = 'douyin_topic_profiles'
    __table_args__ = (
        UniqueConstraint('platform', 'sec_user_id', 'period_start', 'period_end', name='uq_douyin_topic_profile_period'),
    )

    id = Column(Integer, primary_key=True, comment='主键ID')
    platform = Column(String(32), nullable=False, default='dy', index=True, comment='平台标识')
    sec_user_id = Column(String(255), nullable=False, index=True, comment='账号安全标识')
    period_start = Column(String(10), nullable=False, comment='分析周期开始日期 YYYY-MM-DD')
    period_end = Column(String(10), nullable=False, comment='分析周期结束日期 YYYY-MM-DD')
    top_tags = Column(Text, comment='高频标签JSON')
