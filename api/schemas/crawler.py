...
class CrawlerStartRequest(BaseModel):
    ...
    start_page: int = 1
    start_time: str = ""  # Content publish start date, YYYY-MM-DD, empty means no limit
    end_time: str = ""  # Content publish end date, YYYY-MM-DD, empty means no limit
...
class SchedulerConfigRequest(BaseModel):
    """Scheduled crawler configuration."""
    enabled: bool = False
    times: list[str] = Field(default_factory=list)
    crawler: CrawlerStartRequest


class SchedulerStatusResponse(BaseModel):
    """Scheduled crawler runtime status."""
    enabled: bool
    times: list[str]
