...
class DouYinCrawler(AbstractCrawler):
    ...
    async def _get_or_create_context_page(self) -> Page:
        """Reuse a stable MediaCrawler tab inside the shared browser context."""
        pages = self.browser_context.pages
        for page in pages:
            try:
                if await page.evaluate("() => window.name") == "MediaCrawler":
                    return page
            except Exception:
                continue
        ...
        try:
            await page.evaluate("() => { window.name = 'MediaCrawler'; }")
        except Exception:
            pass
        return page
            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                for post_item in posts_res.get("data"):
                    try:
                        ...
                        continue
                    if not self._is_in_collection_time_range(aweme_info):
                        continue
    ...
    async def get_specified_awemes(self):
        ...
        valid_aweme_id_list = []
        detail_map = await self._get_aweme_details_with_retry(aweme_id_list)
        for aweme_id in aweme_id_list:
            aweme_detail = detail_map.get(aweme_id)
            if aweme_detail is not None and self._is_in_collection_time_range(aweme_detail):
                valid_aweme_id_list.append(aweme_id)
                await douyin_store.update_douyin_aweme(aweme_item=aweme_detail)
                await self.get_aweme_media(aweme_item=aweme_detail)
        await self.batch_get_note_comments(valid_aweme_id_list)

    def _is_in_collection_time_range(self, item: Dict) -> bool:
        """Check whether an item is inside the configured publish date range."""
        start_date = str(config.START_TIME or "").strip()
        end_date = str(config.END_TIME or "").strip()
        if not start_date and not end_date:
            return True

        raw_ts = item.get("create_time")
        if raw_ts is None:
            return True
        try:
            ts = int(raw_ts)
        ...
        if start_date:
            start_ts = utils.get_unix_time_from_time_str(f"{start_date} 00:00:00")
            if start_ts and ts < start_ts:
                return False
        if end_date:
            end_ts = utils.get_unix_time_from_time_str(f"{end_date} 23:59:59")
            if end_ts and ts > end_ts:
                return False
        return True

    async def _get_aweme_details_with_retry(self, aweme_ids: List[str]) -> Dict[str, Any]:
        """Fetch video details and retry once after a delay when risk control blocks them."""
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)

        async def fetch(aweme_id: str):
            detail = await self.get_aweme_detail(aweme_id, semaphore)
            return aweme_id, detail

        results: Dict[str, Any] = {}
        pairs = await asyncio.gather(*[fetch(aweme_id) for aweme_id in aweme_ids])
        for aweme_id, detail in pairs:
            results[aweme_id] = detail

        failed_ids = [aweme_id for aweme_id, detail in results.items() if not detail]
        if failed_ids and config.ENABLE_RISK_CONTROL_RETRY:
            utils.logger.warning(
                f"[DouYinCrawler] Detected {len(failed_ids)} videos failed, "
                f"waiting {config.RISK_CONTROL_RETRY_DELAY_SECONDS}s before retrying"
            )
            await asyncio.sleep(config.RISK_CONTROL_RETRY_DELAY_SECONDS)
            retry_pairs = await asyncio.gather(*[fetch(aweme_id) for aweme_id in failed_ids])
            for aweme_id, detail in retry_pairs:
                if detail:
                    results[aweme_id] = detail

        remaining_failed = [aweme_id for aweme_id, detail in results.items() if not detail]
        if remaining_failed:
            utils.logger.warning(
                f"[DouYinCrawler] Still failed after retry: {len(remaining_failed)} videos skipped"
            )
        return results
    ...
    async def get_creators_and_videos(self) -> None:
        ...
        for creator_url in config.DY_CREATOR_ID_LIST:
            ...
            video_ids = [
                ...
                if video_item.get("aweme_id") and self._is_in_collection_time_range(video_item)
            ]
    ...
    async def fetch_creator_video_detail(self, video_list: List[Dict]):
        ...
        """
        video_list = [
            post_item for post_item in video_list
            if post_item.get("aweme_id") and self._is_in_collection_time_range(post_item)
        ]
        aweme_ids = [post_item.get("aweme_id") for post_item in video_list]
        detail_map = await self._get_aweme_details_with_retry(aweme_ids)
        for aweme_id in aweme_ids:
            aweme_item = detail_map.get(aweme_id)
