...
class CDPBrowserManager:
    ...
    def _read_recorded_port(self) -> Optional[int]:
        try:
            return int(self._port_file.read_text(encoding="utf-8").strip())
        except Exception:
            return None

    def _save_recorded_port(self, port: int) -> None:
        try:
    ...
    def _clear_recorded_port(self) -> None:
        try:
    ...
    def _register_cleanup_handlers(self):
        ...
        def sync_cleanup():
            ...
            if config.AUTO_CLOSE_BROWSER and self.launcher and self.launcher.browser_process:
        ...
        def signal_handler(signum, frame):
            ...
            if config.AUTO_CLOSE_BROWSER:
                ...
                utils.logger.info(f"[CDPBrowserManager] Received signal {signum}, keeping browser process")
    ...
    async def launch_and_connect(
        ...
        try:
            ...
            # Reuse a program-managed browser when a valid CDP endpoint is still alive.
            reusable_port = await self._find_reusable_browser_port()
            if reusable_port:
                self.debug_port = reusable_port
                try:
                    ...
                    utils.logger.info(
                        f"[CDPBrowserManager] Reusing existing browser on port {self.debug_port}"
                    )
                    return browser_context
                except Exception as reuse_error:
                    utils.logger.warning(
                        f"[CDPBrowserManager] Failed to reuse browser on port {self.debug_port}: {reuse_error}"
                    )
                    await self.cleanup()
    ...
    async def _test_cdp_connection(
        self,
        debug_port: int,
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        self._save_recorded_port(self.debug_port)

    async def _get_browser_websocket_url(
        self,
        debug_port: int,
        log_error: bool = True,
        timeout: float = 10,
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"http://localhost:{debug_port}/json/version", timeout=timeout
                )
