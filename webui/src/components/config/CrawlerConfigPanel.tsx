...
import { Clock3, Database, Globe, KeyRound, MessageSquare, Play, Save, Square, X } from 'lucide-react'
...
export function CrawlerConfigPanel() {
  ...
  const saveSchedule = useMutation({
    ...
    onError: (error: Error) => {
      toast.error(`${t('schedule.failed')}: ${error.message}`)
    },
  })
  ...
  useEffect(() => {
    ...
    setScheduleTimesText(schedulerStatus.times.join(', '))
  }, [schedulerStatus])
  ...
  const scheduleSaveDisabled = saveSchedule.isPending || (scheduleEnabled && (scheduleTimes.length === 0 || invalidScheduleTimes.length > 0))
      ...
      {config.platform === 'dy' && (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-[220px_1fr_auto] gap-4 items-end">
            <div className="flex items-center gap-3 rounded-lg border border-cyber-border-subtle bg-cyber-bg-tertiary/30 p-3">
              <Checkbox
                checked={scheduleEnabled}
                onCheckedChange={(checked) => setScheduleEnabled(checked === true)}
                disabled={isDisabled || saveSchedule.isPending}
              />
              <div className="min-w-0">
                <p className="text-xs font-mono text-cyber-text-primary">{t('schedule.enable')}</p>
                {schedulerStatus?.next_run_at ? (
                  <p className="text-[10px] text-cyber-text-muted mt-0.5">
                    {t('schedule.nextRun')}: {new Date(schedulerStatus.next_run_at).toLocaleString()}
                  </p>
                ) : (
                  <p className="text-[10px] text-cyber-text-muted mt-0.5">{t('schedule.never')}</p>
                )}
              </div>
            </div>
            ...
            <Field label={t('schedule.times')} hint={t('schedule.timesHint')}>
              <Input
                ...
                onChange={(e) => setScheduleTimesText(e.target.value)}
                placeholder="14:00, 18:00"
                disabled={isDisabled || saveSchedule.isPending || !scheduleEnabled}
                className="h-9 text-xs font-mono"
              />
              {invalidScheduleTimes.length > 0 && (
                <p className="text-[10px] text-cyber-neon-orange font-mono">
                  {t('schedule.invalid')}: {invalidScheduleTimes.join(', ')}
                </p>
              )}
            </Field>
