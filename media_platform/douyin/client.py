    ...
    async def get_all_user_aweme_posts(self, sec_user_id: str, callback: Optional[Callable] = None):
        ...
        while posts_has_more == 1:
            try:
                ...
                if not config.ENABLE_RISK_CONTROL_RETRY:
                    raise
                utils.logger.warning(
                    f"[DouYinClient.get_all_user_aweme_posts] Risk control triggered while fetching page, "
                ...
                try:
                    ...
                    utils.logger.warning(
                        f"[DouYinClient.get_all_user_aweme_posts] Still blocked after retry, "
                        f"stopping pagination with {len(result)} videos already fetched: {e}"
                    )
                    break
            ...
            if config.START_TIME and aweme_list:
                start_ts = utils.get_unix_time_from_time_str(f"{config.START_TIME} 00:00:00")
                reached_start_boundary = False
                if start_ts:
                    for item in aweme_list:
                        try:
                            item_ts = int(item.get("create_time", 0))
                        except (TypeError, ValueError):
                            continue
                        if item_ts > 10 ** 12:
                            item_ts = int(item_ts / 1000)
                ...
                if reached_start_boundary:
                    posts_has_more = 0
                    utils.logger.info(
                        f"[DouYinClient.get_all_user_aweme_posts] Reached publish date before {config.START_TIME}, stopping pagination"
                    )
