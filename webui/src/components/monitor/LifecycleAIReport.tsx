...
import { BrainCircuit, CheckCircle2, ChevronDown, History, Lightbulb, RefreshCw, Sparkles, TriangleAlert } from 'lucide-react'
...
export function LifecycleAIReport({ accountId, timeRange, postLimit, posts }: LifecycleAIReportProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (force: boolean) => {
      if (typeof accountId !== 'number') throw new Error(t('lifecycleReport.singleAccountRequired'))
      return monitorApi.analyzeLifecycle({ account_id: accountId, time_range: timeRange, post_limit: postLimit, force })
    },
    onSuccess: (response) => {
      setSelectedId(response.data.id)
      queryClient.setQueryData<AIAnalysisResponse<AILifecycleAnalysisResult>[]>(historyKey, (current = []) => [response.data, ...current.filter((item) => item.id !== response.data.id)])
      toast.success(t('lifecycleReport.completed'))
    },
    onError: (error: Error) => toast.error(`${t('lifecycleReport.failed')}: ${analysisErrorMessage(error)}`),
  })
    ...
    <div className="mx-auto w-full max-w-[1100px] space-y-5 pb-6">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <details className="group relative"><summary className="flex h-9 cursor-pointer list-none items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs text-slate-600 hover:bg-slate-50 [&::-webkit-details-marker]:hidden"><History className="h-3.5 w-3.5" />{t('lifecycleReport.history')}<ChevronDown className="h-3.5 w-3.5 transition-transform group-open:rotate-180" /></summary><div className="absolute right-0 z-30 mt-2 max-h-80 w-[360px] overflow-y-auto rounded-lg border border-slate-200 bg-white p-2 shadow-xl">{history.length > 0 ? history.map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`w-full rounded-md px-3 py-2 text-left ${current?.id === item.id ? 'bg-purple-50' : 'hover:bg-slate-50'}`}><div className="flex items-center justify-between text-[11px] text-slate-700"><span>{formatDateTime(item.updated_at)}</span><span className="font-mono text-[10px] text-slate-400">{item.model || '-'}</span></div><div className="mt-1 line-clamp-2 text-[10px] leading-4 text-slate-500">{item.result?.overall_summary || item.status}</div></button>) : <div className="px-3 py-8 text-center text-xs text-slate-400">{t('lifecycleReport.noHistory')}</div>}</div></details>
          <Button type="button" disabled={disabled} onClick={() => mutation.mutate(Boolean(current))} className="h-9 bg-[#722ED1] px-4 text-xs text-white hover:bg-[#5A1FA8]">{mutation.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{mutation.isPending ? t('lifecycleReport.analyzing') : current ? t('lifecycleReport.reanalyze') : t('lifecycleReport.analyze')}</Button>
        </div>
      </header>
      ...
      {mutation.isPending ? <LifecycleReportSkeleton postCount={postLimit} /> : current ? <LifecycleReportContent current={current} posts={posts} insightMap={insightMap} /> : typeof accountId === 'number' && configured ? <LifecycleEmpty onAnalyze={() => mutation.mutate(false)} /> : null}
    </div>
  )
}
...
function RichText({ text, className = '' }: { text: string; className?: string }) {
  return <span className={className}>{String(text || '').split(/(\*\*[^*]+\*\*)/g).map((part, index) => part.startsWith('**') && part.endsWith('**') ? <strong key={`${part}-${index}`} className="font-semibold text-slate-900">{part.slice(2, -2)}</strong> : <span key={`${part}-${index}`}>{part}</span>)}</span>
}
...
function LifecycleReportContent({ current, posts, insightMap }: { current: AIAnalysisResponse<AILifecycleAnalysisResult>; posts: MonitorDashboardPost[]; insightMap: Map<string, AILifecycleAnalysisResult['post_insights'][number]> }) {
  const { t } = useTranslation('config')
  const result = current.result
  const summary = result.overall_summary || t('lifecycleReport.fallbackSummary', { count: result.source_post_count || posts.length })
  const stageObservation = result.stage_observation || t('lifecycleReport.fallbackStage')
  const tablePosts = posts.slice(0, 50)
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-5 py-4"><div><div className="text-base font-semibold text-slate-900">{t('lifecycleReport.detailTitle')}</div><p className="mt-1 text-xs text-slate-500">{t('lifecycleReport.detailDescription')}</p></div><span className="text-xs text-slate-400">{t('lifecycleReport.detailCount', { count: tablePosts.length })}</span></div>
